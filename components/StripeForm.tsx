...
const PayUPaymentForm = ({
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (response.data?.formData && response.data?.url) {
        toast.dismiss();
        const { formData, url } = response.data;
    ...
    <div className="p-6 md:p-8 max-w-2xl mx-auto bg-white rounded-xl shadow-lg border border-gray-200">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Click below to complete your payment securely through PayU.
        </p>
      </div>
      {paymentError && (
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        ...
        <p className="text-xs text-blue-700">
          You will be redirected to PayU's secure payment page to complete your purchase.
        </p>
      </div>
      <button
        type="button"
